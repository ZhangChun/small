...
//				left: e + "px"
			...
			m = l.length;
			if (0 === m) return alert("Missing icons config!"),
			!1;
			for (var n, o, p, q, r, s, t, u, v, w, x, y = '<div class="emoji_container" id="emoji_container_' + j + '">',
			z = '<div class="emoji_content">',
			A = '<div class="emoji_tab" style="' + (1 !== m || k ? "": "display:none;") + '"><div class="emoji_tab_prev"></div><div class="emoji_tab_list"><ul>', B = 0; m > B; B++) if (o = l[B].name || "group" + (B + 1), p = l[B].path, q = l[B].maxNum, r = l[B].excludeNums, s = l[B].file || ".jpg", t = l[B].placeholder || "#em" + (B + 1) + "_{alias}#", u = l[B].alias, v = l[B].title, w = 0, p && q) {
			...
			y += D,
			a(y).appendTo(a("body")),
			f = a(i).offset().top + a(i).outerHeight() + 5,
			g = a(i).offset().left,
			...
			if ("DIV" === a.nodeName) {
				if (a.focus(), b.getSelection) {
					if (f = b.getSelection(), f.getRangeAt && f.rangeCount) {
						...
						var l = k.firstChild;
						g.insertNode(k),
						j && (g = g.cloneRange(), g.setStartAfter(j), e ? g.setStartBefore(l) : g.collapse(!0), f.removeAllRanges(), f.addRange(g))
					}
				} else if ((f = c.selection) && "Control" !== f.type) {
					var m = f.createRange();
					m.collapse(!0),
					f.createRange().pasteHTML(html),
					e && (g = f.createRange(), g.setEndPoint("StartToStart", m), g.select())
				}
	...
	d.prototype = {
		_init: function() {
			...
			if (i > 0) for (var k = 0; i > k; k++) if (a = h[k].path, b = h[k].file || ".jpg", c = h[k].placeholder, d = h[k].alias, a) if (d) {
				...
				f = new RegExp(e, "gm"),
				g.$content.html(g.$content.html().replace(f,
				function(c, d) {
					var e = j[d];
					return e ? '<img class="emoji_icon" src="' + a + e + b + '"/>': c
				}))
			} else e = c.replace(new RegExp("{alias}", "gi"), "(\\d+?)"),
